## 1. 产品概述
本项目定名为「灵岩谱曲台」，复刻 Google Bach Doodle 网页版的交互界面，包含交互式五线谱/钢琴卷帘 UI。
- 主要目的是实现用户在网格上点击放置音符的功能，并提供一个“生成”按钮。点击按钮后，系统会模拟 AI 模型的行为，在空白处生成其他声部的和声（配器）。当前版本（v0.1.0-alpha）暂时使用前端占位算法模拟生成，重点在于跑通界面交互和视觉反馈。
- 该界面的核心价值在于为后续正式接入真实的 AI 音乐推理大模型（如 Coconet）提供一个高可用、美观的前端壳子。
...
| 主页面 | 操作按钮区 | 触发和声生成的占位逻辑（后续替换为真实 AI），并渲染回乐谱区 |

## 3. 核心流程
用户进入页面 -> 在乐谱网格上点击输入主旋律 -> 点击“生成”按钮 -> 系统基于占位算法（或真实的AI大模型推理）为每个时间步填充缺少的声部音符 -> 界面以不同颜色高亮显示生成的新音符。
