...
用户可以在 16 步、四小节的乐谱网格上绘制主旋律，系统会自动将旋律吸附到敦煌音阶（1-2-3-♯4-5-6）与节奏位置。播放或生成和声前，系统会把玩家放下的音自动撑长到同小节内的下一个音或小节末尾，让输入保持简单，同时得到更接近乐谱逻辑的时值。点击“生成和声”后，前端会向 AI 后端发送旋律数据，获取多声部和声建议，并支持在浏览器中与主旋律一同回放。
